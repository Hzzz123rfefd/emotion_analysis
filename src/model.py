...
class ModelPretrainForEmotionAnalysis(nn.Module):
    def __init__(
        self, 
        ...
        self.classifier_head = nn.Sequential(
            ...
            nn.ReLU(),                            # 激活函数
            nn.Dropout(p=0.3),                    # Dropout 层，防止过拟合
            nn.Linear(hidden_size, self.num_class)  # 输出层，映射到类别数
        )
    ...
    def forward(self,input:dict):
        ...
        sequence_output = outputs.last_hidden_state[:, 0, :]  # shape: [batch_size, seq_len, hidden_size]
    ...
    def load_pretrained(self, save_model_dir):
        ...
        self.classifier_head.load_state_dict(torch.load(save_model_dir + "/classifier_head.pth"))

    def save_pretrained(self,  save_model_dir):
        self.model.save_pretrained(save_model_dir) 
        torch.save(self.classifier_head.state_dict(), save_model_dir + "/classifier_head.pth")
